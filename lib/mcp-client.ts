...
export class MCPClient {
  ...
  async connect(): Promise<void> {
    try {
      // Connecting to MCP server silently
  ...
  async disconnect(): Promise<void> {
    if (this.client) {
      try {
        ...
        // Disconnected from MCP server successfully
      } catch (error) {
        // Error during disconnect (non-critical)
      }
    }
    this.client = null;
    this.transport = null;
  ...
  async getServerInfo(): Promise<any> {
    ...
    try {
      ...
      // Server connection established with capabilities
      return {
